...
### The Update Rule (Equation 7.5)

```
Q(S_τ, A_τ) ← Q(S_τ, A_τ)  +  α  ×  [ G_{τ:τ+n}  −  Q(S_τ, A_τ) ]
                                         ─────────────────────────────
                                                  TD error
                                         (how wrong the current Q was)
```

| Piece | Meaning |
|---|---|
| `τ = t − n + 1` | The state being updated — always **n steps behind** current time t |
| `α` | Learning rate — how far to move toward the new target |
| `G − Q` | TD error — positive means Q was too low, negative means too high |

#### Concrete example — our 4×4 gridworld

```
Path:    (0,0) → (0,1) → (0,2) → (0,3) → (1,3) → (2,3) → GOAL
Rewards:   -1      -1      -1      -1      -1      +10
Settings:  n=4,  α=0.5,  γ=1.0

Updating state (0,2) at τ=2:
  Rewards from τ+1=3 to τ+4=6:  R₃=-1  R₄=-1  R₅=-1  R₆=+10
  τ+4 = 6 = T  →  episode ended  →  NO bootstrap
  G  =  -1 + -1 + -1 + 10  =  +7

  Q((0,2), Right) += 0.5 × (7 - 0)  =  +3.5   ✓ matches paper
```

### Q-values After Episode 1

| State | Monte Carlo | 1-step Sarsa | n-step (n=4) |
|---|---|---|---|
| (0,0) Right | **+2.5** | -0.5 | -2.0 |
| (0,1) Right | **+3.0** | -0.5 | -2.0 |
| (0,2) Right | **+3.5** | -0.5 | **+3.5** |
| (0,3) Down | **+4.0** | -0.5 | **+4.0** |
| (1,3) Down | **+4.5** | -0.5 | **+4.5** |
| (2,3) Down | **+5.0** | **+5.0** | **+5.0** |
| **Useful updates** | **6 / 6** | **1 / 6** | **4 / 6** |

---

## 8. Gridworld Results

### Learning Curves — All Episodes

![Learning Curves](graph1_learning_curves.png)

- **Monte Carlo** starts at −140 — high variance from random early episodes
- **1-step Sarsa** starts best — converges fastest with dense -1 step reward
- **n-step Sarsa** sits between — better than MC online, close to Sarsa on dense reward

### One Episode — What Each Algorithm Learned

![One Episode Grids](one_episode_grids.png)

- **Monte Carlo** — all 6 path cells updated (6/6)
- **1-step Sarsa** — only last cell updated (1/6)
- **n-step Sarsa** — last 4 cells updated (4/6)

### Q-values and Update Targets

![One Episode Values](one_episode_values.png)

Left panel: Q-values after episode 1 — matches hand-solved paper calculation exactly.
Right panel: Update target G at each state — shows how much goal signal each algorithm carries.

### Full Comparison Graph

![Comparison](comparison_n4.png)
...
pip install pygame numpy matplotlib colorama

# Interactive pygame game — watch agent learn live
python gaming.py

# One episode walkthrough — matches paper calculation
python one_episode.py

# All episodes — learning curves + Q-table heatmaps
python part1_all_episodes.py

# Interactive terminal learner — step by step with sparkline
python part1_interactive.py
```
...
| Key | Action |
|---|---|
| `Q` / `W` / `E` | Switch algorithm (MC / 1-step Sarsa / n-step) |
| `SPACE` | Pause / Resume |
| `ENTER` | Run one episode |
| `↑` / `↓` | Speed up / Slow down |
| `+` / `-` | Increase / Decrease n |
| `R` | Reset Q-table |
| `D` | Toggle dark / light mode |
| `G` | Save comparison graph |
| `S` | Save single algorithm graph |

### Change settings (Section 1 of gaming.py)

```python
ROWS    = 4      # grid size
COLS    = 4
R_STEP  = -1     # step penalty  (0 = sparse reward)
N       = 4      # n for n-step Sarsa
